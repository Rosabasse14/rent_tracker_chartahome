  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const newProof = {
        ...
        unitName: user?.role === 'TENANT' ? (language === 'fr' ? "Mon Unité" : "My Unit") : "Unknown Unit",
        amount: parseFloat(newPayment.amount),
        period: new Date().toLocaleString(language === 'en' ? 'en-US' : 'fr-FR', { month: 'long', year: 'numeric' }),
      ...
      toast.success(t.upload_success_msg);
    } catch (error) {
      console.error("Upload error:", error);
      toast.error(t.upload_error_msg);
    }
  };

  const handleStatusChange = (id: string, newStatus: 'paid' | 'rejected') => {
    verifyPayment(id, newStatus);
    const statusText = newStatus === 'paid' ? t.paid : t.rejected;
    toast.success(`${t.status_update_msg} ${statusText}`);
  };
          ...
          <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="MTN MoMo">{t.method_momo}</SelectItem>
                        <SelectItem value="Orange Money">{t.method_orange}</SelectItem>
                        <SelectItem value="Bank Transfer">{t.method_bank}</SelectItem>
                ...
                <Button className="w-full" onClick={handleUpload}>{t.submit_payment_btn}</Button>
              </div>
            </DialogContent>
          </Dialog>
        )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-lg flex items-center gap-2">
                    {t.payment_prefix} {proof.proofNumber}
                  ...
                  <p className="text-sm text-muted-foreground">
                    {language === 'fr' ? 'Soumis le' : 'Submitted'} {proof.submittedAt}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-primary uppercase">
                    {proof.paymentMethod === 'Cash' ? t.method_cash :
                      proof.paymentMethod === 'MTN MoMo' ? t.method_momo :
                        proof.paymentMethod === 'Orange Money' ? t.method_orange :
                          proof.paymentMethod === 'Bank Transfer' ? t.method_bank : proof.paymentMethod || t.method_cash}
                  </p>
                </div>
              </div>
            </div>
